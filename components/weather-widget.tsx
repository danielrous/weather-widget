...
// weather effects
import { SunnyParticles } from "./weather/SunnyParticles";
// icons
import { Sun, CloudRain, Thermometer } from "lucide-react";
import CanvasCloudSystem from "./weather/CanvasCloudSystem";
import WindStreaks from "./weather/WindStreaks";
        ...
        <div className="z-12 relative grid min-h-[100dvh] grid-cols-1 flex-col items-center justify-center p-2 transition-all duration-300 ease-in-out lg:flex w-full">
          <div className="bg-muted h-full w-full overflow-clip rounded-3xl">
            <div className="relative h-full w-full">
              <Card
                ref={containerRef}
                className="absolute top-1/6 left-1/2 rounded-3xl border-border outline-ring/50 -translate-x-1/2 w-80 aspect-[3/4] md:w-100 lg:w-180 lg:aspect-[4/3] md:aspect-[3/4] overflow-hidden cursor-pointer transition-all duration-500"
              >
                {/* Animated Background */}
                ...
                <div className="absolute bottom-0 left-0 w-full z-100 px-3 pb-3 flex justify-between gap-2">
                  {(Object.keys(weatherStates) as WeatherState[]).map((weather) => {
                    ...
                    return (
                      <Button
                        key={weather}
                        variant={currentWeather === weather ? "secondary" : "primary"}
                        size="lg"
                        onClick={() => setCurrentWeather(weather)}
                        className="w-full rounded-xl flex-1 p-2"
                      >
                        <StateIcon size={16} />
                      </Button>
                    );
